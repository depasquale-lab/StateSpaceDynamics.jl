...
function plot_lds_benchmark(
    fp::AbstractString,
    sp::AbstractString=joinpath(@__DIR__, "results", "lds_benchmark_results.svg"),
)
    ...
    package_markers = Dict(
        "Dynamax" => :circle, "pykalman" => :square, "StateSpaceDynamics.jl" => :diamond
    )
            ...
            for impl in impls
                ...
                if !isempty(impl_data)
                    plot!(
                        p,
                        impl_data.seq_len,
                        impl_data.median_time;
                        label=impl,
                        marker=package_markers[impl],
                        linewidth=2,
                        color=package_colors[impl],
                    )
                end
            end
