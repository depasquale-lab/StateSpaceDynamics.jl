...
# Load data
df = CSV.read("results/glmhmm_benchmark_results.csv", DataFrame)

# Prepare columns
df.seq_len = categorical(df.seq_len, ordered=true)
df.implementation = categorical(df.implementation)
df.config = string.(df.latent_dim) .* "x" .* string.(df.input_dim)
...
savefig(p1, "results/glmhmm_bench.svg")
