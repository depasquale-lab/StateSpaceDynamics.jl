...
"""
function _isvalid_obs_model(obs_model::GaussianObservationModel{T}, obs_dim::Int, latent_dim::Int) where T
    # Check C matrix
...
"""
function _isvalid_obs_model(obs_model::PoissonObservationModel{T}, obs_dim::Int, latent_dim::Int) where T
    # Check C matrix
    if size(obs_model.C) != (obs_dim, latent_dim)
        @warn "C matrix size $(size(obs_model.C)) ≠ expected ($obs_dim, $latent_dim)"
        return false
