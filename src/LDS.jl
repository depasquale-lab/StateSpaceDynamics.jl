# export statement
export LDS, KalmanFilter, KalmanSmoother, loglikelihood, PLDS
export RTSSmoother,
    DirectSmoother, KalmanSmoother, KalmanFilterEM!, loglikelihood, marginal_loglikelihood
...
function RTSSmoother(l::LDS, y::AbstractArray)
    ...
    for t in (T - 1):-1:1
        ...
        x_smooth[t, :] =
            x_filt[t, :] + J[t, :, :] * (x_smooth[t + 1, :] - l.A * x_filt[t, :])
...
function DirectSmoother(l::LDS, y::Matrix{<:Real})
    ...
    p_smooth, inverse_offdiag = StateSpaceDynamics.block_tridiagonal_inverse(
        -sub, -main, -super
    )
...
"""
function _sufficient_statistics(
    smoother::RTSSmoothing, μ::Matrix{<:Real}, P::Array{<:Real}, J::Array{<:Real}
)
    T = size(μ, 1)
...
"""
function _sufficient_statistics(
    smoother::DirectSmoothing, μ::Matrix{<:Real}, P::Array{<:Real}, Ptt1::Array{<:Real}
)
    T = size(μ, 1)
    # Initialize sufficient statistics
    E_z = zeros(T, size(μ, 2))
...
function update_initial_state_covariance!(
    l::DynamicalSystem, E_z::Matrix{<:Real}, E_zz::Array{<:Real}
)
...
function update_A!(l::DynamicalSystem, E_zz::Array{<:Real}, E_zz_prev::Array{<:Real})
    ...
    if l.fit_bool[3]
        l.A =
            dropdims(sum(E_zz_prev; dims=1); dims=1) *
            pinv(dropdims(sum(E_zz[1:(end - 1), :, :]; dims=1); dims=1))
    end
end
...
function update_Q!(l::DynamicalSystem, E_zz::Array{<:Real}, E_zz_prev::Array{<:Real})
    if l.fit_bool[4]
        ...
        for n in 2:N
            sum_expectations +=
                E_zz[n, :, :] - (E_zz_prev[n, :, :] * l.A') - (l.A * E_zz_prev[n, :, :]') +
                (l.A * E_zz[n - 1, :, :] * l.A')
        end
...
function update_R!(l::LDS, E_z::AbstractArray, E_zz::AbstractArray, y::AbstractArray)
    if l.fit_bool[6]
        ...
        for n in 1:N
            sum_terms +=
                (y[n, :] * y[n, :]') - (l.H * (y[n, :] * E_z[n, :]')') -
                ((y[n, :] * E_z[n, :]') * l.H') + (l.H * E_zz[n, :, :] * l.H')
        end
...
function M_Step!(
    ...
    E_z::Matrix{<:Real},
    E_zz::Array{<:Real},
    E_zz_prev::Array{<:Real},
    y_n::AbstractMatrix{<:Real},
)
    ...
    return update_R!(l, E_z, E_zz, y_n)
end

function KalmanFilterEM!(
    l::LDS,
    y::AbstractArray,
    max_iter::Int=1000,
    tol::Float64=1e-12,
    smoother::SmoothingMethod=RTSSmoothing(),
)
...
function Gradient(l::LDS, y::AbstractArray, x::AbstractArray)
    ...
    grad[1, :] =
        (l.A' * inv_Q * (x[2, :] - l.A * x[1, :])) +
        (l.H' * inv_R * (y[1, :] - l.H * x[1, :])) - (inv_p0 * (x[1, :] - l.x0))
    # calulate the gradient up until the last time step
    Threads.@threads for t in 2:(T - 1)
        grad[t, :] =
            (l.H' * inv_R * (y[t, :] - l.H * x[t, :])) -
            (inv_Q * (x[t, :] - l.A * x[t - 1, :])) +
            (l.A' * inv_Q * (x[t + 1, :] - l.A * x[t, :]))
    end
    # calculate the gradient for the last time step
    grad[T, :] =
        (l.H' * inv_R * (y[T, :] - l.H * x[T, :])) - (inv_Q * (x[T, :] - l.A * x[T - 1, :]))
...
"""
    PLDS(A, C, Q, D, d, x₀, p₀, obs_dim, latent_dim, fit_bool)
...
plds = PLDS(A, C, Q, D, d, x₀, p₀, refractory_period, obs_dim, latent_dim, fit_bool)
"""
function PLDS(;
    A::AbstractMatrix{<:Real}=Matrix{Float64}(undef, 0, 0),
    ...
    return PLDS(
        A, C, Q, D, log_d, b, x0, p0, refractory_period, obs_dim, latent_dim, fit_bool
    )
end

function logposterior_nonthreaded(x::AbstractMatrix{<:Real}, plds::PLDS, y::Matrix{<:Real})
    ...
    for t in 1:T
        pygivenx +=
            (y[t, :]' * ((plds.C * x[t, :]) + (plds.D * s[t, :]) + d)) -
            sum(exp.((plds.C * x[t, :]) + (plds.D * s[t, :]) + d))
    end
...
"""
    logposterior(x::Matrix{<:Real}, plds::PLDS, y::Matrix{<:Real})
...
- `plds::PLDS`: The Poisson Linear Dynamical System model.
- `y::Matrix{<:Real}`: The observed data.
...
```
"""
function logposterior(x::AbstractMatrix{<:Real}, plds::PLDS, y::Matrix{<:Real})
...
"""
    loglikelihood(x::Array{<:Real}, plds::PLDS, y::Array{<:Real})
...
- `plds::PLDS`: The Poisson LDS model.
...
# Example
"""
function loglikelihood(x::Array{<:Real}, plds::PLDS, y::Array{<:Real})
...
"""
function Q_initial_obs(
    x0::Vector{<:Real}, sqrt_p0::Matrix{<:Real}, E_z::Array{<:Real}, E_zz::Array{<:Real}
)
    # reparametrize p0
    p0 = sqrt_p0 * sqrt_p0'
    # Compute Q
    ...
    for k in 1:trials
        Q_val +=
            -0.5 * (
                logdet(p0) + tr(
                    pinv(p0) * (
                        E_zz[k, 1, :, :] - (E_z[k, 1, :] * x0') - (x0 * E_z[k, 1, :]') +
                        (x0 * x0')
                    ),
                )
            )
...
"""
function Q_state_model(
    A::Matrix{<:Real}, sqrt_Q::Matrix{<:Real}, E_zz::Array{<:Real}, E_zz_prev::Array{<:Real}
)
    # reparametrize Q
    Q = sqrt_Q * sqrt_Q'
    Q_inv = pinv(Q)
...
"""
    Q_function(plds::PLDS, E_z::Array{<:Real}, E_zz::Array{<:Real}, E_zz_prev::Array{<:Real}, y::Array{<:Real})
...
function directsmooth(plds::PLDS, y::Matrix{<:Real})
    ...
    p_tt1 = cat(
        reshape(
            zeros(plds.latent_dim, plds.latent_dim), 1, plds.latent_dim, plds.latent_dim
        ),
        p_tt1;
        dims=1,
    )
...
"""
function smooth(plds::PLDS, y::Array{<:Real})
    if isempty(plds.b)
...
# Example
"""
function Gradient(x::Matrix{<:Real}, plds::PLDS, y::Matrix{<:Real})
    ...
    grad[1, :] =
        ((y[1, :]' * plds.C)' - sum(plds.C' * exp.(plds.C * x[1, :] + d); dims=2)) +
        (plds.A' * inv_Q * (x[2, :] - (plds.A * x[1, :] + plds.b[1, :]))) -
        (inv_p0 * (x[1, :] - plds.x0))
    ...
    for t in 2:(T - 1)
        grad[t, :] =
            (
                (y[t, :]' * plds.C)' -
                sum(plds.C' * exp.(plds.C * x[t, :] + plds.D * s[t, :] + d); dims=2)
            ) - (inv_Q * (x[t, :] - (plds.A * x[t - 1, :] + plds.b[t, :]))) +
            (plds.A' * inv_Q * (x[t + 1, :] - (plds.A * x[t, :] + plds.b[t + 1, :])))
    end
...
"""
    Hessian(x::Matrix{<:Real}, plds::PLDS, y::Matrix{<:Real})
...
"""
function Hessian(x::Matrix{<:Real}, plds::PLDS, y::Matrix{<:Real})
    # convert log_d to d i.e. non-log space
    d = exp.(plds.log_d)
...
"""
function E_Step(plds::PLDS, y::Array{<:Real})
...
"""
    update_x0!(plds::PLDS, x_smooth::Array{<:Real})

Update the initial state of a Poisson Linear Dynamical System (PLDS) model.

# Arguments
- `plds::PLDS`: The PLDS model to update.
...
"""
function update_initial_state_mean!(plds::PLDS, E_z::Array{<:Real})
...
"""
update_initial_state_covariance!(plds::PLDS, E_zz::Array{<:Real}, E_z::Array{<:Real})

Update the initial covariance matrix `p0` of a `PLDS` model.
...
# Details
This function updates the initial covariance matrix `p0` of a `PLDS` model based on the expected values of the latent variables. The update is performed only if the `fit_bool[2]` flag is set to `true`.
...
"""
function update_initial_state_covariance!(
    plds::PLDS, E_zz::Array{<:Real,3}, E_z::Array{<:Real,3}
)
...
"""
    update_A_plds!(plds::PLDS, E_zz::Array{<:Real}, E_zz_prev::Array{<:Real})

Update the transition matrix `A` of a Poisson Linear Dynamical System (PLDS) model.

# Arguments
- `plds::PLDS`: The PLDS model object.
...
# Example
"""
function update_A_plds!(plds::PLDS, E_zz::Array{<:Real}, E_zz_prev::Array{<:Real})
    # update the transition matrix
    if plds.fit_bool[3]
...
"""
    update_Q_plds!(plds::PLDS, E_zz::Array{<:Real}, E_zz_prev::Array{<:Real})

Update the process noise covariance matrix Q for a Poisson Linear Dynamical System (PLDS).

# Arguments
- `plds::PLDS`: The Poisson Linear Dynamical System object.
...
# Example
"""
function update_Q_plds!(plds::PLDS, E_zz::Array{<:Real}, E_zz_prev::Array{<:Real})
...
"""
    update_b!(plds::PLDS, x_smooth::Array{<:Real})
...
# Examples
"""
function update_b!(plds::PLDS, x_smooth::Array{<:Real})
...
function update_observation_model!(
    plds::PLDS, E_z::Array{<:Real}, P_smooth::Array{<:Real}, y::Array{<:Real}
)
    ...
    if plds.fit_bool[6]
        ...
        # update the parameters
        plds.C = reshape(
            result.minimizer[1:(plds.obs_dim * plds.latent_dim)],
            plds.obs_dim,
            plds.latent_dim,
        )
...
function fit!(plds::PLDS, y::Array{<:Real}, max_iter::Int=100, tol::Float64=1e-3)
    # create a variable to store the log-likelihood
    ec_lls = []
    ll_prev = -Inf
    # create a progress bar
    prog = Progress(max_iter; desc="Fitting PLDS via LaPlace EM: ")
...
# Examples
"""
function sample(plds::PLDS, T::Int64, K::Int64)
    # Convert log_d to d i.e. non-log space
    d = exp.(plds.log_d)
    # Pre-allocate arrays
    ...
    for k in 1:K
        ...
        for t in 2:T
            ...
            y[k, t, :] =
                rand.(Poisson.(exp.((plds.C * x[k, t, :]) + (plds.D * spikes) + d)))
        end
    end
    return x, y
end
