...
"""
    loglikelihood(lds, data)
# Inputs-aware marginal (observed-data) log-likelihood of a Gaussian LDS,
# computed by running the Kalman filter and summing the one-step-ahead
# predictive densities. Mirrors `loglikelihood(lds, y)` but adds the dynamics
# input `B·ux_{t-1}` to the state prediction and subtracts the observation
# input `D·uy_t` from the innovation, so a model fit with controls is scored on
# the same footing as the input-bearing null baselines.
"""
function loglikelihood(
    lds::LinearDynamicalSystem{T,SM,OM}, data::Data{T}
) where {T<:Real,SM<:GaussianStateModel{T},OM<:GaussianObservationModel{T}}
    A = lds.state_model.A
    Q = lds.state_model.Q
    b = lds.state_model.b
    ...
    R = lds.obs_model.R
    d = lds.obs_model.d
    Dmat = lds.obs_model.D

    y = data.y
    ux = data.ux
    uy = data.uy
    obs_dim, tsteps, ntrials = size(y)
    latent = lds.latent_dim
    ...
    # Pre-allocate buffers (reused across trials and time steps).
    x_p = Vector{T}(undef, latent)
    x_f = Vector{T}(undef, latent)
    P_p = Matrix{T}(undef, latent, latent)
    P_f = Matrix{T}(undef, latent, latent)
    tmp_DD = Matrix{T}(undef, latent, latent)
    innov = Vector{T}(undef, obs_dim)
    Si_e = Vector{T}(undef, obs_dim)
    Smat = Matrix{T}(undef, obs_dim, obs_dim)
    PCt = Matrix{T}(undef, latent, obs_dim)
    SiPCt = Matrix{T}(undef, obs_dim, latent)
    ...
    for n in 1:ntrials
        ...
        for t in 1:tsteps
            # Prediction (t=1: prediction == prior). Dynamics input `B·ux_{t-1}`
            # kicks the state forward from t-1 to t (matches the sampler).
            if t == 1
                x_p .= x0
                P_p .= P0
            else
                ...
                x_p .+= b
                if ux_dim > 0
                    @views mul!(x_p, B, ux[:, t - 1, n], one(T), one(T))
                end
                mul!(tmp_DD, A, P_f)
                mul!(P_p, tmp_DD, A')
                P_p .+= Q
            ...
            @views innov .= y[:, t, n] .- innov .- d
            if uy_dim > 0
                @views mul!(innov, Dmat, uy[:, t, n], -one(T), one(T))
            end
