...
function test_grouped_smooth_accepts_a_single_trial_matrix()
    ...
    x1, P1 = smooth(fitted, y[1]; depends_on=(C=[:a], d=[:a]))
    ...
    xs, Ps = smooth(fitted, [y[1]]; depends_on=(C=[:a], d=[:a]))
...
function test_grouped_matrix_normal_priors()
    ...
    om_free.depends_on = (C=labels, d=labels, R=labels)
...
function pd_poisson_slds(labels; K::Int=2)
    ldss = map(1:K) do k
        ...
        labels === nothing || (om.depends_on = (C=labels, d=labels))
        return LinearDynamicalSystem(;
            state_model=sm,
            obs_model=om,
            latent_dim=PD_LATENT_DIM,
...
function test_grouped_slds_rand_needs_a_label_for_one_trial()
    ...
    z, x, y = rand(StableRNG(1012), slds, 30; depends_on=(C=[:s2], d=[:s2], R=[:s2]))
