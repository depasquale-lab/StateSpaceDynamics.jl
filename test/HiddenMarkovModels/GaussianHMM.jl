...
function test_sample_non_f64()
    
    model = GaussianHMM(K=1, output_dim=3)
    μ = [0.0,  1.0,  -1.0]
    Σ = 0.5 * I(3)
    model.B[1] = GaussianEmission(output_dim=3, μ=μ, Σ=Σ)

    n = 100
    true_labels, Y = StateSpaceDynamics.sample(model; n=n)

    @test length(true_labels) == n
    @test size(Y) == (3, n)
    @test eltype(Y) == Float64
end

function test_fit_non_f64()
    ...
    Y_int = Int.(round.(all_data))
    Y_f32 = Float32.(all_data) 

    #create two models with different types 
    model_int = GaussianHMM(; K=2, output_dim=2)
    ll_int = StateSpaceDynamics.fit!(model_int, Y_int; max_iters=100)

    @test all(eltype(ll_int) .== Float64)
    @test eltype(model_int.B[1].μ) == Float64
    @test eltype(model_int.B[1].Σ) == Float64

    model_f32 = GaussianHMM(; K=2, output_dim=2)
    ll_f32 = StateSpaceDynamics.fit!(model_f32, Y_f32)

    @test all(eltype(ll_f32) .== Float64)
    @test eltype(model_f32.B[1].μ) == Float64
