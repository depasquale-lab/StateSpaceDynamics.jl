...
function test_SwitchingGaussianRegression_SingleState_fit()
    ...
    # Create Switching Regression Model
    true_model = SwitchingGaussianRegression(K=1, input_dim=3, output_dim=1, include_intercept=true)
    ...
    # Try to fit a new model to the data
    test_model = StateSpaceDynamics.SwitchingGaussianRegression(K=1, input_dim=3, output_dim=1, include_intercept=true)
    ll = StateSpaceDynamics.fit!(test_model, data, Φ)
...
function test_trialized_SwitchingGaussianRegression()
    ...
    model = SwitchingGaussianRegression(K=2, input_dim=1, output_dim=1, include_intercept=true)
    model.B[1].β = [100; 100;;]
    model.B[2].β = [-100; -100;;]
    model.B[1].Σ = [2.0;;]
    model.B[2].Σ = [3.0;;]
