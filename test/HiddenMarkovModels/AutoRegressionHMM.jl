...
function test_AutoRegressionHMM()
    ...
    # check the β values
    similar_β = false
    if isapprox(true_model.B[1].β, est_model.B[1].β, atol=0.1) && isapprox(true_model.B[2].β, est_model.B[2].β, atol=0.1)
        # the emissions were paired one to one
        similar_β = true
    elseif isapprox(true_model.B[2].β, est_model.B[1].β, atol=0.1) && isapprox(true_model.B[1].β, est_model.B[2].β, atol=0.1)
        # the emissions were paired one to one
        similar_β = true
    else
        similar_β = false
