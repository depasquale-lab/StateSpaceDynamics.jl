...
function test_null_heldout_ll_matches_plugin_gaussian(rng=MersenneTwister(1))
    ...
    ref = _null_ref_ll(Y_te, ones(1, size(Y_te, 2)), reshape(null.d, obs_dim, 1), null.R)
...
function test_null_inputs_recovers_signal(rng=MersenneTwister(3))
    ...
    @test isapprox(null.D, D_true; atol=5e-2, rtol=0, norm=x -> maximum(abs, x))
...
function test_null_var_recovers_true_F(rng=MersenneTwister(4))
    ...
    @test isapprox(null.F, F_true; atol=5e-2, rtol=0, norm=x -> maximum(abs, x))
    @test null.d ≈ d_true atol = 5e-2
    @test isapprox(null.R, R_true; atol=2e-2, rtol=0, norm=x -> maximum(abs, x))
