...
function test_Bernoulli_ll_gradient()
    ...
    # initialize model
    est_model = BernoulliRegression()
    est_model.β = [0.0, 0.0, 0.0]

    function objective(β, X, w)
        temp_model = BernoulliRegression()
        temp_model.β = β
        temp_model.λ = est_model.λ
        return -StateSpaceDynamics.loglikelihood(temp_model, X, y, w) +
               (temp_model.λ * sum(temp_model.β .^ 2))
    end

    grad = ForwardDiff.gradient(x -> objective(x, X, ones(1000)), est_model.β)

    # calculate the gradient manually
    grad_analytic = StateSpaceDynamics.gradient!([0.0, 0.0, 0.0], est_model, X, y)
    ...
    grad_analytic = StateSpaceDynamics.gradient!([0.0, 0.0, 0.0], est_model, X, y, weights)
    @test isapprox(grad, grad_analytic, atol=1e-6)
